...
df_drop = df.drop(*['ArrTime','ActualElapsedTime', 'AirTime', 'TaxiIn', 'Diverted', 'CarrierDelay', 'WeatherDelay', 'NASDelay', 'SecurityDelay', 'LateAircraftDelay'])
df_drop.columns
...
df_drop = df.drop(*['DepTime','CRSDepTime'])
...
#%%
int_type=["DepTime", "ArrDelay", "DepDelay", "Distance"]
from pyspark.sql.types import IntegerType

for i in int_type:
    clean_df = clean_df.withColumn(i, clean_df[i].cast(IntegerType()))
...
clean_df = clean_df.drop(*["DepTime", "CRSDepTime", "FlightNum"])
#%%
...
for i in integer_cols:
    ...
    train_scaled = pipeline.fit(train_scaled).transform(train_scaled).withColumn(i+"_Scaled", unlist(i+"_Scaled")).drop(i+"_Vect")                                                                
...
"CORRELATION"
#solo funciona con variables tipo int
intCols = [x for (x, dataType) in train_scaled.dtypes if dataType =="int"]
for i in intCols:
    print(i)
    print(train_scaled.stat.corr(i,"FlightNum"))
